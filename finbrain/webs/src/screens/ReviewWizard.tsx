...
import { SankeyChart, num, shortMoney, type SankeyLinkInput, type SankeyNodeInput } from '../lib/finance'
...
function AssetFlowOverview() {
  ...
  const flow = useMemo(() => (valuation.data ? buildAssetFlow(valuation.data) : null), [valuation.data])

  return (
    <div className="fb-card" style={{ padding: 16, marginBottom: 18, display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
          <div className="fb-num" style={{ color: 'var(--text-secondary)', fontSize: 12.5 }}>
            {valuation.data ? shortMoney(valuation.data.total_assets, displayCurrency) : '—'}
          </div>
          <Button
            size="sm"
            variant="secondary"
      ...
      {valuation.isLoading ? (
        ...
        <>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', fontSize: 11.5, color: 'var(--text-tertiary)' }}>
        ...
        <div style={{ height: 220, display: 'grid', placeItems: 'center', color: 'var(--text-tertiary)', fontSize: 13 }}>
          {valuation.isError ? '资产流向加载失败' : '暂无可展示的资产流向'}
        </div>
      )}
      {shareOpen && flow ? (
        <AssetFlowShareModal
          ...
          totalAssets={valuation.data?.total_assets ?? '0'}
...
function buildAssetFlow(valuation: Valuation): AssetFlowData {
  ...
  for (const position of valuation.positions ?? []) {
    const value = num(position.market_value_display) ?? 0
    if (value <= 0) continue
    items.push({
  ...
  const maxLeaves = 10
  const topLeafIDs = new Set(
    [...leafTotals.entries()]
      .sort((a, b) => b[1] - a[1])
      .slice(0, maxLeaves)
      .map(([id]) => id),
  )
  const compactedCount = Math.max(0, leafTotals.size - topLeafIDs.size)
...
async function renderAssetFlowShareImage({
  ...
  const layout = layoutAssetFlowForCanvas(flow.nodes, flow.links, {
    width,
    height: 350,
    x: 44,
    y: 142,
  ...
  for (const link of layout.links) {
    ...
    ctx.bezierCurveTo(link.x0 + link.bend, link.y0, link.x1 - link.bend, link.y1, link.x1, link.y1)
    ctx.strokeStyle = shareColorFromToken(link.color, accent)
    ctx.lineWidth = Math.max(1.4, link.width)
    ctx.stroke()
  }
  ctx.restore()
  ...
  for (const node of layout.nodes) {
    ...
    const labelOnLeft = node.column === layout.maxColumn
    const labelX = labelOnLeft ? node.x0 - 8 : node.x1 + 8
    const midY = (node.y0 + node.y1) / 2
    const label = shareTrimLabel(node.label, labelOnLeft ? 24 : 18)
...
type CanvasSankeyNode = SankeyNodeInput & {
  value: number
  x0: number
  x1: number
  y0: number
...
function layoutAssetFlowForCanvas(
  nodes: SankeyNodeInput[],
  links: SankeyLinkInput[],
  opts: { width: number; height: number; x: number; y: number; nodeW: number; gap: number },
) {
  const nodeIDs = new Set(nodes.map((n) => n.id))
  const incoming = new Map<string, number>()
  const outgoing = new Map<string, number>()
  const safeLinks = links.filter((l) => l.value > 0 && Number.isFinite(l.value) && nodeIDs.has(l.source) && nodeIDs.has(l.target))
  ...
  const layoutNodes: CanvasSankeyNode[] = nodes
    .map((node) => ({
      ...node,
  ...
  const maxColumn = Math.max(0, ...layoutNodes.map((n) => n.column))
  const columns = new Map<number, CanvasSankeyNode[]>()
  for (const node of layoutNodes) {
    const col = columns.get(node.column) ?? []
    col.push(node)
    columns.set(node.column, col)
  }
  const scales = [...columns.values()].map((col) => {
    const total = col.reduce((sum, n) => sum + n.value, 0)
    const gaps = Math.max(0, col.length - 1) * opts.gap
    return total ? Math.max(0.0001, (opts.height - gaps) / total) : 1
  })
  ...
  for (const [column, col] of columns) {
    ...
    let y = opts.y + Math.max(0, (opts.height - used) / 2)
    const x = opts.x + ((opts.width - opts.x * 2 - opts.nodeW) * column) / Math.max(1, maxColumn)
  ...
  const layoutLinks = safeLinks
    .filter((link) => byID.has(link.source) && byID.has(link.target))
    .sort((a, b) => {
      const sourceA = byID.get(a.source)!
      const sourceB = byID.get(b.source)!
      const targetA = byID.get(a.target)!
      const targetB = byID.get(b.target)!
      return sourceA.column - sourceB.column || sourceA.y0 - sourceB.y0 || targetA.y0 - targetB.y0
    })
    .map((link) => {
      const source = byID.get(link.source)!
      const target = byID.get(link.target)!
      const width = link.value * scale
      const y0 = source.sourceCursor + width / 2
      const y1 = target.targetCursor + width / 2
      source.sourceCursor += width
      target.targetCursor += width
      const x0 = source.x1
      const x1 = target.x0
      return {
        x0,
        ...
        y0,
        y1,
        bend: Math.max(44, Math.abs(x1 - x0) * 0.54),
        width,
        color: link.color ?? source.color ?? 'var(--accent)',
      }
    })

  return { nodes: layoutNodes, links: layoutLinks, maxColumn }
}
...
function shareTrimLabel(label: string, maxUnits: number) {
  let units = 0
  let out = ''
