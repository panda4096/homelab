...
- 维护页以**表格为主**，承担单条新增 / 编辑 / 删除与过滤 / 排序；**批量导入走后端 API（§4.10.1），此页不提供批量录入界面**
- 价格 / 汇率的历史走势可视化用**折线图**（单值序列，**非 K 线**——每天仅一个价）；完整的标的历史价格图与基准叠加曲线属趋势分析（§4.16 / §6.13），在 **P5** 实现。维护页本阶段可选提供一条"按 symbol 的迷你折线 + 缺口提示"作为录入审阅辅助（发现录错的尖刺、缺数据的日期段），不替代表格
