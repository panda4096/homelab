...
价格按 **(symbol, 日期) 时间序列**存储：同一标的可有任意多条历史价格，一条 = 某天一个价（**单值，非 OHLC**）。当前估值取"不晚于估值日的最近一条"（见 §6.2 / §6.14）；历史价格点越多，趋势图（§4.16）与基准曲线（§4.18）越平滑——但**不要求每日连续**，稀疏点经插值也能出图。

录入方式：

- **单条录入**（维护页 §7.18）：标的、日期、价格、币种；以及修订 / 删除单条
- **批量导入（仅后端 API，无前端表单）**：用于 agent / 脚本一次性补录历史价格序列，见 §4.10.1。维护页只做单条 CRUD，不提供批量录入界面

约束：

- `(symbol, price_date, currency)` 唯一（重复键 = 覆盖，即修订）
- `price` > 0
- 日期可为**任意日期**（含未来，如已知结算价 / 指数点位）；只校验日期格式，**不套用**余额 / 持仓快照的"今天 + 7"上限（那是 §4.2 专用规则）

#### 4.10.1 批量导入 API（价格 / 汇率共用）

为支持 agent 或脚本一次性补录历史序列，后端提供批量导入接口（**仅 API，维护页不暴露 UI**）：

- **输入**：价格行数组 `[{symbol, price_date, price, currency, source?, note?}, …]`；汇率同理 `[{base_currency, quote_currency, rate_date, rate, source?, note?}, …]`
- **语义**：整批 **upsert**，按各自唯一键覆盖已存在的同键行（幂等，可安全重跑）
- **原子性**：先整批校验，**任一行不合法则整批拒绝**（单事务），返回逐行错误（行号 + 原因），不产生半导入状态
- **上限**：单批行数与列表上限一致（见工程约定，默认 5000 行）
- `source` 缺省 `manual`；agent / 自动源导入时填来源标记（如 `agent` / `yahoo` / `futu`）
- **返回**：成功导入 / 覆盖的行数

### 4.11 汇率录入

汇率同样按 **(币种对, 日期) 时间序列**存储；当前折算与历史折算的取数规则见 §6.3。

录入方式：

- **单条录入**（维护页 §7.18）：基准币种、报价币种、日期、汇率；以及修订 / 删除单条
- **批量导入（仅后端 API）**：见 §4.10.1（价格与汇率共用同一套批量导入约定）

约束：

- `(base_currency, quote_currency, rate_date)` 唯一（重复键 = 覆盖）
- `rate` > 0
- 日期可为任意日期，只校验格式（同 §4.10）
...
- 点密度：趋势按截面取"不晚于该日的最近一条"插值（§6.14），**不要求每日连续价格点**；点越密曲线越平滑。手录场景下用 §4.10.1 批量导入补足历史价格即可
...
若某账户/某标的在 `on_date` 之前没有任何快照，则**反向投影其最早的一条快照**：把该最早快照的余额/数量视为在 `on_date` 当时已持有（持仓按 `on_date` 当日历史价估值），使净值趋势曲线在首次盘点之前不掉为 0。

> 实现取舍：早期设计为"按 0 处理（视为当时不存在）"，现行实现改为"反向投影最早快照"，以避免趋势曲线左端在首次盘点前出现断崖。代价是首次盘点之前的历史截面会展示尚未真正持有的资产，属已知偏离；如未来需要严格的"当时不存在"语义，应在趋势展示层单独提供开关，而非改动默认截面口径。对应 `currentCashRows` / `currentPositionRows` 的 `latest_balance` / `latest_position` CTE。
...
三个并列的维护页（同一导航入口下切换；价格点位并入「标的」详情，不再单列价格 tab——一个 symbol 的元数据与它的价格序列在同一处管理）：
...
- 维护页以**表格为主**做单条 CRUD + 过滤/排序;**批量导入走后端 API（§4.10.1），此页不提供批量录入界面**。
- 价格 / 汇率历史用**折线图**（单值序列,**非 K 线**——每天仅一个价/汇率）,配密度/缺口提示（点偏少或日期段缺数据时提示用批量导入补足）。完整的趋势主图与基准叠加曲线属趋势分析（§4.16 / §6.13）,在 **P5** 实现;本页折线为录入审阅辅助,不替代表格。
